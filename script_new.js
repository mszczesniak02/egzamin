...
function startQuiz() {
    ...
    document.getElementById('setup-view').classList.remove('hidden'); // Keep header visible? 
    // Actually the structure is: setup-view contains the header AND the loading status.
    // We want the header and stats to persist?
    // The current HTML structure hides #setup-view entirely.
    // Let's move the header and stats OUT of #setup-view in HTML or just handle visibility differently.
    // For now, let's just make sure stats are visible when quiz starts.
    
    // Refactor: Move header and stats out of #setup-view in next step? 
    // Or just hack it here:
    // The user wants the counter "od włączenia strony".
    ...
    // We need to hide the "Wybierz folder" part but keep the Title and Stats?
    // Currently #setup-view has the Title.
    // If we hide #setup-view, we lose the title.
    // Let's NOT hide #setup-view, but hide its children?
    // Or better: Modify HTML to separate Header/Stats from "Setup Content".
    
    // Let's assume we fixed HTML structure.
    // But for now, let's just show the stats bar.
    updateStatsUI();
...
// ... (renderCategorySelection, rejectCategory remain unchanged) ...
    ...
    for (let line of lines) {
        ...
        if (collectingAnswer) {
            ...
            if (anyTagRegex.test(line)) {
                collectingAnswer = false;
                // If it's a tag, we let the loop continue? 
                // No, if it matched a tag regex, it would have been caught by the specific checks above.
                // If it matches generic tag regex but NOT specific ones, we should probably stop collecting too.
                continue; 
            }
